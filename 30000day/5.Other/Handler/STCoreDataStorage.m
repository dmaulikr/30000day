...
@implementation STCoreDataStorage
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    ...
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        NSLog(@"%@: Creating persistentStoreCoordinator", [self class]);
...
- (NSManagedObjectContext *)managedObjectContext
{
    ...
    if (coordinator)
    {
        NSLog(@"%@: Creating managedObjectContext", [self class]);
...
- (NSManagedObjectContext *)mainThreadManagedObjectContext
{
    ...
    if (coordinator)
    {
        NSLog(@"%@: Creating mainThreadManagedObjectContext", [self class]);
...
- (void)maybeSave:(int64_t)currentPendingRequests
{
    ...
    if ([[self managedObjectContext] hasChanges])
    {
        ...
        {
            NSLog(@"%@: Triggering save (pendingRequests=%lli)", [self class], currentPendingRequests);
        ...
        else
        {
            ...
            {
                NSLog(@"%@: Triggering save (unsavedCount=%lu)", [self class], (unsigned long)unsavedCount);
...
- (void)maybeSave
{
    ...
    [self maybeSave:OSAtomicAdd64(0, &pendingRequests)];
}
...
- (void)executeBlock:(dispatch_block_t)block
{
    ...
    OSAtomicIncrement64(&pendingRequests);
    dispatch_sync(storageQueue, ^{ @autoreleasepool {
        ...
        dispatch_async(storageQueue, ^{ @autoreleasepool {
            ...
            [self maybeSave:OSAtomicDecrement64(&pendingRequests)];
