...
- (void)initViewAndSubViews {
    ...
    if (error) {
        [self showToast:[NSString stringWithFormat:@"没有摄像头%@", error.localizedDescription]];
        return;
    }
    ...
    [STDataHandler sendPushMessageWithCurrentUserId:STUserAccountHandler.userProfile.userId
                                        ...
                                        messageType:@1
                                            success:^(BOOL success) {
                                                ...
                                                [STDataHandler sendUserInformtionWithUserId:userId success:^(UserInformationModel *model) {
