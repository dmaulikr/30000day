...
-(void)markDates:(NSArray *)dates {
    ...
    if (self.markedDates == nil) {  //健身数据与预约删除完毕后需要清除三角标记
        [self setNeedsDisplay];
        return;
    } else {
        ...
        for (int i = 0; i<[dates count]; i++) {
            [colors addObject:[UIColor colorWithHexString:@"0x383838"]]; // original: colorWithHexString:@"0x383838"
        }
...
-(void)reset {
    ...
    [self updateSize];
    [self setNeedsDisplay];
    [delegate calendarView:self switchedToMonth:[currentMonth month] swithchedToYear:[currentMonth year] targetHeight:self.calendarHeight animated:NO];
}
...
-(void)showNextMonth {
    ...
    if ([delegate respondsToSelector:@selector(calendarView:switchedToMonth:swithchedToYear:targetHeight: animated:)]) [delegate calendarView:self switchedToMonth:[currentMonth month] swithchedToYear:[currentMonth year] targetHeight:self.calendarHeight animated:YES];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (CGRectContainsPoint(rectArrowLeft, touchPoint)) {
        ...
        [self reset];
        if ((todayMonth!=currentMonthIndex) && [delegate respondsToSelector:@selector(calendarView:switchedToMonth:swithchedToYear:targetHeight:animated:)]) [delegate calendarView:self switchedToMonth:[currentMonth month] swithchedToYear:[currentMonth year] targetHeight:self.calendarHeight animated:NO];
    }
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    [labelCurrentMonth sizeThatFits:CGSizeMake(kVRGCalendarViewWidth-68, 40)];
    ...
    CGContextSetFillColorWithColor(context, [UIColor whiteColor].CGColor);//colorWithHexString:@"0xf3f3f3"
    ...
    CGContextMoveToPoint(context, 0, gridHeight+kVRGCalendarViewTopBarHeight - 1);
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        if (selectedDate && i==selectedDateBlock) {
            ...
            if (self.isFirstInit) {
                ...
                CGContextSetFillColorWithColor(context, [UIColor redColor].CGColor); // 选中的颜色colorWithHexString:@"0xaaaaaa"
            ...
            }else if (!self.isFirstInit && todayBlock == i ){
                
                CGRect rectangleGrid = CGRectMake(targetX+0.5f,targetY+0.7f,kVRGCalendarViewDayWidth+2,kVRGCalendarViewDayHeight+2);
                
                CGContextAddRect(context, rectangleGrid);
                
                CGContextSetFillColorWithColor(context, [UIColor lightGrayColor].CGColor); // original: colorWithHexString:@"0x383838"  ( 当天的背景色)
        ...
        [date drawInRect:CGRectMake(targetX, targetY+kVRGCalendarViewDayWidth/2.00f-9.50f, kVRGCalendarViewDayWidth, kVRGCalendarViewDayHeight) withFont:[UIFont fontWithName:@"HelveticaNeue-Bold" size:17] lineBreakMode:NSLineBreakByClipping alignment:NSTextAlignmentCenter];
    }
...
//        CGContextFillPath(context);
        ...
        int X = targetColumn * (kVRGCalendarViewDayWidth+2) + kVRGCalendarViewDayWidth*3.00f/4.00f;
        int Y = kVRGCalendarViewTopBarHeight + targetRow * (kVRGCalendarViewDayHeight+2);
        int magin = kVRGCalendarViewDayWidth / 4.00f;
    
        CGContextBeginPath(context);
        CGContextMoveToPoint(context,X,Y);
        CGContextAddLineToPoint(context,(targetColumn+1)*(kVRGCalendarViewDayWidth+2),Y);
        CGContextAddLineToPoint(context,(targetColumn+1)*(kVRGCalendarViewDayWidth+2),Y+magin);
        CGContextAddLineToPoint(context,X,Y);
        CGContextSetFillColorWithColor(context,
                                       [UIColor colorWithHexString:@"0x383838"].CGColor); // original: colorWithHexString:@"0x383838"
        CGContextFillPath(context);
    }
}
